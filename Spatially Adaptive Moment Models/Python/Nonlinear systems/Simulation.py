...
class SpatiallyAdaptiveSimulation1D(Simulation):
    ...
    def runSimulation(self,tend):
        ...
        deltaX = (self.x2 - self.x1)/self.resolution #TODO: include the possibility of nonuniform grids
        g = 1

        cellCentersX = self.mesh.createMesh() 
        self.calculateBoundaryInterfaces()
        print(self.boundaryInterfaces_Discretized)

        values = self.getInitialConditions(cellCentersX)
        ...
        while t < tend:
            ...
            previousValues[self.resolution+1] = self.updateBoundaryConditions(previousValues[self.resolution])
            ...
            for i in range(rightBoundary_subDomain+1,self.resolution+1):
                fluctuationPlus = self.spatialDiscretization.computeFluctuation(
                    previousValues[i-1],
                    previousValues[i],
                    systemMatrixRight,
    ...
    def calculateBoundaryInterfaces(self):
        self.boundaryInterfaces_Discretized.append(round((self.boundaryInterfaces[0] - self.x1)/(self.x2 - self.x1)*self.resolution))
    ...
    def getInitialConditions(self,cellCentersX):
            ...
            for i in range(rightBoundary_subDomain,self.resolution):
    ...
    def computeBreakdownCriteria(self,values):
        relativeValueLastMoment = np.zeros(self.resolution)
