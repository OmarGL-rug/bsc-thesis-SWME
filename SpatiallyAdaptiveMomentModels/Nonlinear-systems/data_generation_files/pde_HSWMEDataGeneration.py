...
class SWME1D(PDE):
    ...
    def get_initial_values(self,
        ...
        if initial_condition == 'hDamLarge':
            x0 = -0.2
            x1 = 0.2
            if x0 < position < x1:
                initial_values[0] = 1.0
            else:
                initial_values[0] = 3.0
        if initial_condition == 'hDamMid':
            x0 = -0.2
            x1 = 0.2
            if x0 < position < x1:
                initial_values[0] = 1.0
        ...
        elif initial_condition == 'hDamSmall':
            x0 = -0.2
            x1 = 0.2
            if x0 < position < x1:
                initial_values[0] = 1.0
        ...
        elif initial_condition == 'hSmoothSteep':
            initial_values[0] = 1 + 1.0*np.exp(-10*(position+0.1)**2)
        elif initial_condition == 'hSmoothMid':
            initial_values[0] = 1 + 0.5*np.exp(-5*(position+0.1)**2)
        elif initial_condition == 'hSmoothSmall':
            initial_values[0] = 1 + 0.2*np.exp(-2*(position+0.1)**2)
        
        mag = 0
        if initial_velocity_magnitude == 'uFast':
            mag = 1.0
        elif initial_velocity_magnitude == 'uMid':
            mag = 0.5
        elif initial_velocity_magnitude == 'uSlow':
            mag = 0.2

        if initial_velocity == 'uConst':
            initial_values[1] = mag*initial_values[0]
        elif initial_velocity == 'uVarXnotvarZ':
            if start < position < end:
                initial_values[1] = mag*(position-start)/range_factor*initial_values[0]
        elif initial_velocity == 'uVarZnotvarX':
            initial_values[1] = mag*initial_values[0]
            if order > 0:
                initial_values[2] = -0.5*initial_values[1]
            if order > 1:
                initial_values[3] = 0.5*initial_values[1]
            if order > 2:
                initial_values[4] = 0.5*initial_values[1]
            if order > 3:
                initial_values[5] = -0.5*initial_values[1]
            if order > 4:
                initial_values[6] = -0.5*initial_values[1]
            if order > 5:
                initial_values[7] = 0.5*initial_values[1]
        elif initial_velocity == 'uVarXvarZ':
            if start < position < end:
                initial_values[1] = mag*(position-start)/range_factor*initial_values[0]
                if order > 0:
                    initial_values[2] = -0.5*initial_values[1]
